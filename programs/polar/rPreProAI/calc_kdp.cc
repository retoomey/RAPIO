...
// -------------------------------------------------------------------------
void
median_1D_filter(int filter_length, const std::vector<float>& in, std::vector<float>& out)
{
  int n        = in.size();
...
// -------------------------------------------------------------------------
// Main Implementation
//    I've split the compute_triple_median_Kdp in two so that I can 
//    output the stdPhiDP. Also this saves computation time, because the
//    stdPhiDP does't change based on long or short Kdp.
//
// -------------------------------------------------------------------------
...
compute_std_PhiDP(std::shared_ptr<RadialSet> PhiDP)
{
  ...
  auto Output = PhiDP->Clone();

  Output->setTypeName("StdDifferentialPhase");
  Output->setUnits("None");
  Output->setDataAttributeValue("ColorMap", "StdDev");

  auto& phiData = PhiDP->getFloat2DRef();
  auto& outData = Output->getFloat2DRef();

  // Fill output with missing initially
  Output->getFloat2D()->fill(Constants::MissingData);
  ...
  for (size_t a = 0; a < num_az; ++a) {
    raw_phi.assign(num_gates, 0.0f);
    diff_phi.assign(num_gates, 0.0f);
    stddev_phi.assign(num_gates, 0.0f);
    // Extract radial
    ...
    median_1D_filter(phi_filter_length, raw_phi, single_med_phi);
    median_1D_filter(phi_filter_length, single_med_phi, tmp_phi);
    median_1D_filter(phi_filter_length, tmp_phi, triple_med_phi);
    ...
    for (size_t g = 0; g < num_gates; ++g) {
      outData[a][g] = stddev_phi[g];
    }
  }
  Output->setTypeName("StdDiffPhase");
  return Output; 
}
// -------------------------------------------------------------------------
std::shared_ptr<RadialSet>
...
{
  if (!PhiDP || !CC || !stdPhiDP) {
    fLogSevere("compute_Kdp_postStd: Nullptr provided for PhiDP or CC.");
    return nullptr;
  }
  ...
  if ((num_az != CC->getNumRadials()) || (num_gates != CC->getNumGates())) {
    fLogSevere("compute_Kdp_postStd: Dimension mismatch between PhiDP and CC.");
    return nullptr;
  }
  ...
  if (gateWidthMeters <= 0.0) { 
    fLogSevere("compute_Kdp_postStd: missing gateWidthKMs. using 250m override");
    gateWidthMeters = 250.0; // Failsafe
  }
  double distFirstGateM = PhiDP->getDistanceToFirstGateM();
  ...
  for (size_t a = 0; a < num_az; ++a) {
    ...
    for (size_t g = 0; g < num_gates; ++g) {
      raw_phi[g] = phiData[a][g];
      stddev_phi[g] = stdPhiDP_Data[a][g];
    }
    //need this for later
    median_1D_filter(phi_filter_length, raw_phi, single_med_phi);
  ...
  int                                                KDP_filter_length_meters)
{
  ...
  for (size_t a = 0; a < num_az; ++a) {
    ...
    calc_stddev(phi_filter_length, diff_phi, stddev_phi);
//
//-----NOTE--- The subroutine is split here for performance reasons. 
//  do not use this in realtime. It is for example and ease of use
//----------------------------------------------------------------
