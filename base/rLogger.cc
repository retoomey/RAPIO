#include "rLogger.h"     // The header we just created
#include "rError.h"      // For the StartupException if you refactor the exit(1) in Log::initialize()
#include "rStrings.h"    // For Strings::trim, Strings::toLower, and Strings::TokenScan
#include "rEventLoop.h"  // For EventLoop::addEventHandler in Log::setSeverityString
#include "rEventTimer.h" // For EventTimer base class behavior
#include "rIOURL.h"      // For IOURL::read in LogSettingURLWatcher::action
#include "rEventTimer.h" // Now safe to include here
#include "rIOURL.h"      // Now safe to include here
#include "rSignals.h"    // For Signals::initialize in Log::setSeverity
#include "rFactory.h"    // For Factory<Logger>::introduceLazy and get in Log::initialize
#include "rColorTerm.h"  // If needed for color fallback, though Spdlog handles most colors
...
namespace rapio {
// Define the watcher class internally so it doesn't pollute the global headers
class LogSettingURLWatcher : public EventTimer {
public:
  LogSettingURLWatcher(const URL& aURL, size_t milliseconds)
    : EventTimer(milliseconds, "Log Setting URL Watcher"), myURL(aURL){ }

  URL getURL(){ return myURL; }

  void setURL(const URL& aURL){ myURL = aURL; }
...
Log::initialize()
{
  if (myLog == nullptr) {
    ...
    // fail messages...
    if (myLog == nullptr) {
      throw StartupException("Can't find the spd logging dynamic module (librapiologspd.so).");
    }
