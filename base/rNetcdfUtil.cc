...
int NetcdfUtil::GZ_LEVEL = 6;

/** Our default compression stuff for the c library.  */
int
NetcdfUtil::compressVar(int ncid, int datavar)
{
  int retval;
...
int
NetcdfUtil::getAtt(int ncid, const std::string& name, float * v,
  const int varid)
{
  // FIXME: Could crash if it's a array and not a single...check the SIZE
  int retval;
...
int
NetcdfUtil::getAtt(int ncid, const std::string& name, long * v,
  const int varid)
{
  // FIXME: Could crash if it's a array and not a single...check the SIZE
  int retval;
...
bool
NetcdfUtil::addGlobalAttr(int ncid, const DataType& dt,
...
bool
NetcdfUtil::writeUnitValueList(int ncid, const rapio::DataType& dt)
{
  int retval;
 ...
 */
bool
NetcdfUtil::readUnitValueList(int ncid, rapio::DataType& dt)
{
  int retval, retval2;
...
bool
NetcdfUtil::readSparseBackground(int ncid,
  int                                data_var,
...
void
NetcdfUtil::readDimensionInfo(int ncid,
  const char *                    typeName,
  int *                           data_var,
  int *                           data_num_dims,
  const char *                    dim1Name,
...
/*
 * template bool NetcdfUtil::readSparse2D(int,
 *                                     int,
 *                                     int,
 *                                     int,
 *                                     float,
...
// template<class T>
bool
NetcdfUtil::readSparse2D(int ncid,
  int                        data_var,
  int                        num_x,
  int                        num_y,
  float                      fileMissing,
  ...
  DataType                   & data)
{
  ...
  NetcdfUtil::readSparseBackground(ncid, data_var, backgroundValue);
  ...
  return (true);
} // NetcdfUtil::readSparse2D

/*
 #include <rLatLonHeightGrid.h>
 * template bool NetcdfUtil::readSparse3D(int, int, int, int, int, float, float,
 *    LatLonHeightGrid& data);
 * template<class T>
 * bool NetcdfUtil::readSparse3D(int ncid, int data_var,
